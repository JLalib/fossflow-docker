...
[![GitHub](https://img.shields.io/badge/GitHub-stan--smith%2FFossFLOW-blue?logo=github)](https://github.com/stan-smith/FossFLOW)
[![Docker Hub](https://img.shields.io/badge/Docker%20Hub-stnsmith%2Ffossflow-blue?logo=docker)](https://hub.docker.com/r/stnsmith/fossflow)
...
- 🖼️ **Importa tus propios íconos** - PNG, JPG, SVG custom. Escalado automático
...
1. **ENABLE_SERVER_STORAGE** — `true` para persistir diagramas en servidor (volumen `./diagrams`), `false` para solo localStorage navegador
2. **STORAGE_PATH** — Ruta dentro del contenedor donde guardar diagramas (default: `/data/diagrams`)
3. **NODE_ENV** — `production` para modo producción, `development` para desarrollo
4. **ENABLE_GIT_BACKUP** — `true` para habilitar backup automático a Git (requiere configuración adicional)
5. **Puertos** — Cambia `"80:80"` por `"PUERTO_HOST:80"` si necesitas otro puerto
6. **Volúmenes** — Ajusta `./diagrams` a la ruta deseada en host para persistencia
...
Este proyecto está licenciado bajo **MIT License** — ver el archivo [LICENSE](https://github.com/stan-smith/FossFLOW/blob/main/LICENSE) para detalles.
